...
export function ProtectedRouteClient({ children }: ProtectedRouteProps) {
   ...
   if (role !== "cliente") {
      // Si es superadmin, redirigir a super-admin
      if (role === "superadmin") {
...
export function ProtectedRouteSuperAdmin({ children }: ProtectedRouteProps) {
   const { authStatus, role } = useAuthStore();
   ...
   // ✅ Si está autenticado pero no es superadmin
   if (role !== "superadmin") {
      return (
         <div className="flex h-screen items-center justify-center">
            <div className="text-center space-y-4">
               <h2 className="text-xl font-semibold">Acceso Denegado</h2>
               <p className="text-muted-foreground">Area restringida para Super Admin</p>
               <Navigate to="/admin" replace />
            </div>
         </div>
      );
